...
import { Calendar, Users as UsersIcon, ChevronRight, Archive } from "lucide-react";
...
export default function MemberObreirosSchedulesPage() {
  ...
  const { data: allSchedules = [], isLoading } = useQuery<ScheduleWithAssignments[]>({
    queryKey: ["member-schedules-obreiros", "all", selectedYear],
    queryFn: async () => {
      ...
      for (let i = -1; i <= 3; i++) {
        ...
        // Filtrar apenas escalas de obreiros
        const obreiroSchedules = response.filter(s => s.tipo === "obreiros");
        schedules.push(...obreiroSchedules);
      }
      return schedules;
    },
  });
  ...
  const { proximas, atuais, anteriores } = useMemo(() => {
    ...
    // Se houver escalas "hoje" e ainda não inicializou, ativa a aba "atuais"
    if (!tabInitialized && atuais.length > 0) {
      setObreiroTab("atuais");
          ...
          <Card key={schedule.id}>
            ...
            <CardContent className="space-y-2">
              {(schedule.assignments || []).map((assign, index) => {
                ...
                const obreiroNumber = assign.posicao.split('-')[1] ? parseInt(assign.posicao.split('-')[1]) + 1 : index + 1;
                
                return (
                  <div key={assign.id} className="flex justify-between items-center">
                    <span className="text-sm font-medium">
                      Obreiro {obreiroNumber}:
                    </span>
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-lg text-muted-foreground">Visualize as escalas de obreiros que você faz parte.</p>
      </div>
      ...
      <div className="flex gap-2 mb-6 items-center flex-wrap">
        ...
        {obreiroTab === "proximas" && proximas.length > 3 && (
          <button
      ...
      {isLoading ? (
        ...
        renderScheduleCards(proximas, showAllObreiros ? 999 : 3)
      ) : obreiroTab === "atuais" ? (
